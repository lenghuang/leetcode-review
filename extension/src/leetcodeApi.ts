...
/**
 * Applies a delay before fetching submissions based on the current page and batch settings.
 * @param page The current page number.
 * @param requestsPerBatch The number of requests to make before a break.
 * @param interRequestDelayMs The delay between requests within a batch in milliseconds.
 * @param batchBreakDelayMs The delay after a batch in milliseconds.
 */
const applySubmissionFetchDelay = async (
  page: number,
  requestsPerBatch: number,
  interRequestDelayMs: number,
  batchBreakDelayMs: number
) => {
...
/**
 * Fetches a single page of submissions from the LeetCode API.
 * @param url The URL to fetch.
 * @returns The parsed JSON data from the response.
 * @throws An error if the HTTP response status is not OK (excluding 403).
 */
const fetchSubmissionPage = async (url: string): Promise<any> => {
...
export const enumerateSubmissions = async () => {
  ...
  while (hasNext) {
    ...
    await applySubmissionFetchDelay(
      page,
      requestsPerBatch,
      interRequestDelayMs,
      batchBreakDelayMs
    ...
    try {
      const data = await fetchSubmissionPage(url);
